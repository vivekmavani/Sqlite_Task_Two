...
import 'package:sqlite_flutter_task_two/features/audit/data/models/audit_model.dart';

import 'audit_entity_data_source.dart';
import 'database/audit_db.dart';

class AuditDataSourceImpl extends AuditEntityDataSource {
  final AuditEntityDb auditEntityDb;

  AuditDataSourceImpl({required this.auditEntityDb});
  ...
  Future<List<DisplayModel>> getEntriesCount() async {
    ...
    List<String> datas = [];
    final listoftable = await auditEntityDb.getTableNames();
 ...
 List<DisplayModel> finallist = [];
  ...
  Future<void> getJsonAndInsertAuditEntity() async {
    ...
    final data = AuditModel.fromJson(json.decode(response));
